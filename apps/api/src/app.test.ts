...
describe("CoachOS API", () => {
  ...
  it("updates dashboard state after a client check-in", async () => {
    ...
    expect(response.body.dashboard.atRiskClients.some((alert: { clientId: string }) => alert.clientId === "client_3")).toBe(true);
  });
  ...
  it("supports filtered aggregate queries for clients, plans, and check-ins", async () => {
    ...
    const clients = await request(app).get("/api/clients").query({ status: "paused", search: "Liam" });
  ...
  it("stores editable weight goal settings on the client profile", async () => {
    const app = createApp(await DemoStore.create());
    ...
    expect(updated.status).toBe(200);
    expect(updated.body.weightUnit).toBe("lb");
    expect(updated.body.goalWeight).toBe(145);
    expect(updated.body.goalTimelineMonths).toBe(6);
  });
