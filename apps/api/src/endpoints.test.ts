...
describe("POST /api/clients", () => {
  ...
  it("creates a free trial client with zero monthly price", async () => {
    const app = createApp(await DemoStore.create());

    const res = await request(app).post("/api/clients").send({
      fullName: "Free Trial Client",
      email: "free-trial@example.com",
      goal: "Start onboarding with no paid plan yet",
      status: "trialing",
      monthlyPriceGbp: 0,
      nextRenewalDate: ""
    });
    ...
    expect(res.status).toBe(201);
    expect(res.body.monthlyPriceGbp).toBe(0);
  });
