...
- **Storage:** JSON file (dev) / Supabase Postgres (prod-ready)
...
CoachOS now includes versioned Supabase migrations in `supabase/migrations/`.

Apply them to a Supabase project, then configure the Cloudflare Worker API with:
...
The browser still calls `/api`; the Supabase connection string must stay server-side.
